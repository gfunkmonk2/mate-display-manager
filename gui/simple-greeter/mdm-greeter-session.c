...
static void
on_server_select_user (MdmGreeterClient  *client,
                       const char        *username,
                       MdmGreeterSession *session)
{
        g_debug ("MdmGreeterSession: selecting user %s", username);
        mdm_greeter_login_window_select_user (MDM_GREETER_LOGIN_WINDOW (session->priv->login_window), username);
}
                           ...
                           GError           **error)
{
        ...
        g_spawn_command_line_async ("dbus-send "
                                    ...
                                    "com.ubuntu.BootCurtain.SignalLoaded string:mdm",
                                    NULL);
