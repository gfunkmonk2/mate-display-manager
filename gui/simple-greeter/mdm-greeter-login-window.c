...
void
mdm_greeter_login_window_select_user (MdmGreeterLoginWindow *login_window,
                                      const char            *username)
{
        g_return_if_fail (MDM_IS_GREETER_LOGIN_WINDOW (login_window));

        g_debug ("MdmGreeterLoginWindow: selecting user '%s'", username);

        g_free (login_window->priv->username);
        login_window->priv->username = g_strdup (username);
        if (login_window->priv->user_chooser_loaded) {
                g_debug ("MdmGreeterLoginWindow: activating user '%s'", login_window->priv->username);
                mdm_chooser_widget_set_active_item (MDM_CHOOSER_WIDGET (login_window->priv->user_chooser), username);
        }
}
                 ...
                 MdmGreeterLoginWindow *login_window)
{
        ...
        if (!login_window->priv->user_list_disabled) {
                ...
                if (login_window->priv->username) {
                        g_debug ("MdmGreeterLoginWindow: activating user '%s'", login_window->priv->username);
                        mdm_chooser_widget_set_active_item (MDM_CHOOSER_WIDGET (login_window->priv->user_chooser), login_window->priv->username);
                }
        }
