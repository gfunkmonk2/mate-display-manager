...
#define MDM_DBUS_TYPE_G_OBJECT_PATH_ARRAY (dbus_g_type_get_collection ("GPtrArray", DBUS_TYPE_G_OBJECT_PATH))
...
typedef enum {
        ...
        MDM_USER_MANAGER_SEAT_STATE_GET_SESSION_ID,
        MDM_USER_MANAGER_SEAT_STATE_GET_ID,
        MDM_USER_MANAGER_SEAT_STATE_GET_PROXY,
        MDM_USER_MANAGER_SEAT_STATE_LOADED,
} MdmUserManagerSeatState;

typedef struct
{
        MdmUserManagerSeatState      state;
        char                        *id;
        char                        *session_id;
        union {
...
typedef enum {
        MDM_USER_MANAGER_NEW_SESSION_STATE_UNLOADED = 0,
        MDM_USER_MANAGER_NEW_SESSION_STATE_GET_PROXY,
        MDM_USER_MANAGER_NEW_SESSION_STATE_GET_UID,
        MDM_USER_MANAGER_NEW_SESSION_STATE_GET_X11_DISPLAY,
        MDM_USER_MANAGER_NEW_SESSION_STATE_MAYBE_ADD,
        MDM_USER_MANAGER_NEW_SESSION_STATE_LOADED,
} MdmUserManagerNewSessionState;

typedef struct
{
        MdmUserManager                  *manager;
        MdmUserManagerNewSessionState    state;
        char                            *id;
...
{
        MdmUserManager             *manager;
        MdmUserManagerGetUserState  state;
        MdmUser                    *user;
        char                       *username;
        char                       *object_path;

        DBusGProxyCall             *call;
} MdmUserManagerFetchUserRequest;
...
static void     load_users_manually         (MdmUserManager *manager);
static void     monitor_local_users         (MdmUserManager *manager);
static void     load_seat_incrementally     (MdmUserManager *manager);
static void     unload_seat                 (MdmUserManager *manager);
static void     load_users                  (MdmUserManager *manager);
static void     queue_load_seat_and_users   (MdmUserManager *manager);
static void     monitor_local_users         (MdmUserManager *manager);

static void     load_new_session_incrementally (MdmUserManagerNewSession *new_session);
static void     set_is_loaded (MdmUserManager *manager, gboolean is_loaded);

static void     on_new_user_loaded (MdmUser        *user,
                                    GParamSpec     *pspec,
                                    MdmUserManager *manager);
static void     give_up_and_fetch_user_locally (MdmUserManager                 *manager,
                                                MdmUserManagerFetchUserRequest *request);
static void     fetch_user_locally             (MdmUserManager *manager,
                                                MdmUser        *user,
                                                const char     *username);
static void     fetch_user_incrementally       (MdmUserManagerFetchUserRequest *request);

static void     maybe_set_is_loaded            (MdmUserManager *manager);
                         ...
                         const char     *session_id)
{
        ...
        if (! res) {
                if (error != NULL) {
                        g_debug ("MdmUserManager: Failed to identify the session type: %s", error->message);
                        g_error_free (error);
                } else {
                        g_debug ("MdmUserManager: Failed to identify the session type");
                }
                goto out;
        }
...
_get_login_window_session_id (MdmUserManager *manager)
{
        ...
        can_activate_sessions = mdm_user_manager_can_switch (manager);
...
gboolean
mdm_user_manager_can_switch (MdmUserManager *manager)
{
        gboolean    res;
        gboolean    can_activate_sessions;
        GError     *error;
        ...
        if (!manager->priv->is_loaded) {
                g_debug ("MdmUserManager: Unable to switch sessions until fully loaded");
                return FALSE;
        }
                                        ...
                                        MdmUser        *user)
{
        ...
        ssid = mdm_user_get_primary_session_id (user);
        if (ssid == NULL) {
                goto out;
        }
...
{
        if (manager->priv->is_loaded) {
                g_debug ("MdmUserManager: user changed");
                         ...
                         MdmUserManager *manager)
{
        ...
        g_debug ("MdmUserManager: Found current seat: %s", seat_id);
...
static void
get_seat_id_for_current_session (MdmUserManager *manager)
{
        DBusGProxy      *proxy;
        DBusGProxyCall  *call;
        ...
        call = dbus_g_proxy_begin_call (proxy,
                                        ...
                                        G_TYPE_INVALID);
        if (call == NULL) {
                g_warning ("MdmUserManager: failed to make GetSeatId call");
                goto failed;
        }
...
static gboolean
username_in_exclude_list (MdmUserManager *manager,
                          const char     *username)
{
        GSList   *found;
        gboolean  ret = FALSE;
        ...
        if (username == NULL || (strcmp (username, MDM_USERNAME) == 0)) {
                return TRUE;
        }
...
static void
add_session_for_user (MdmUserManager *manager,
                      MdmUser        *user,
                      const char     *ssid)
{
...
static void
set_has_multiple_users (MdmUserManager *manager,
                        gboolean        has_multiple_users)
{
...
static MdmUser *
create_new_user (MdmUserManager *manager)
{
        MdmUser *user;

        user = g_object_new (MDM_TYPE_USER, NULL);
          ...
          MdmUser        *user)
{
        ...
        object_path = mdm_user_get_object_path (user);
        if (object_path != NULL) {
...
static void
remove_user (MdmUserManager *manager,
             MdmUser        *user)
{
        g_object_ref (user);
        ...
        if (mdm_user_get_object_path (user) != NULL) {
                g_hash_table_remove (manager->priv->users_by_object_path, mdm_user_get_object_path (user));
        }
        g_hash_table_remove (manager->priv->users_by_name, mdm_user_get_user_name (user));
...
static void
on_new_user_loaded (MdmUser        *user,
                    GParamSpec     *pspec,
                    MdmUserManager *manager)
{
        const char *username;
        ...
        username = mdm_user_get_user_name (user);
        ...
        if (username == NULL) {
                ...
                if (object_path != NULL) {
                        g_warning ("MdmUserManager: user has no username "
                                   "(object path: %s, uid: %lu)",
                                   object_path, mdm_user_get_uid (user));
                } else {
                        g_warning ("MdmUserManager: user has no username (uid: %lu)",
                                   mdm_user_get_uid (user));
                }
                g_object_unref (user);
                return;
        }
                              ...
                              MdmUserManager *manager)
{
        ...
        _mdm_user_update_from_object_path (user, object_path);
...
on_new_user_in_accounts_service (DBusGProxy *proxy,
                                 ...
                                 gpointer    user_data)
{
        MdmUserManager *manager = MDM_USER_MANAGER (user_data);
                                 ...
                                 MdmUserManager *manager)
{
        ...
        g_assert (manager->priv->seat.state == MDM_USER_MANAGER_SEAT_STATE_GET_SESSION_ID);
...
static void
get_current_session_id (MdmUserManager *manager)
{
        DBusGProxy      *proxy;
        DBusGProxyCall  *call;
        ...
        call = dbus_g_proxy_begin_call (proxy,
                                        ...
                                        G_TYPE_INVALID);
        if (call == NULL) {
                g_warning ("MdmUserManager: failed to make GetCurrentSession call");
                goto failed;
        }
...
static void
unload_new_session (MdmUserManagerNewSession *new_session)
{
        MdmUserManager *manager;
        ...
        g_slice_free (MdmUserManagerNewSession, new_session);
}

static void
get_proxy_for_new_session (MdmUserManagerNewSession *new_session)
{
        MdmUserManager *manager;
        DBusGProxy     *proxy;
...
on_get_unix_user_finished (DBusGProxy               *proxy,
                           DBusGProxyCall           *call,
                           MdmUserManagerNewSession *new_session)
{
        MdmUserManager *manager;
        GError         *error;
        guint           uid;
        ...
        g_debug ("MdmUserManager: Found uid of session '%s': %u",
                 new_session->id, uid);
...
static void
get_uid_for_new_session (MdmUserManagerNewSession *new_session)
{
        DBusGProxyCall *call;
        ...
        call = dbus_g_proxy_begin_call (new_session->proxy,
                                        ...
                                        G_TYPE_INVALID);
        if (call == NULL) {
                g_warning ("MdmUserManager: failed to make GetUnixUser call");
                goto failed;
        }
                               ...
                               MdmUserManagerFetchUserRequest *request)
{
        ...
        if (! res) {
                if (error != NULL) {
                        g_debug ("MdmUserManager: Failed to find user %s: %s",
                                 request->username, error->message);
                        g_error_free (error);
                } else {
                        g_debug ("MdmUserManager: Failed to find user %s",
                                 request->username);
                }
        ...
        g_debug ("MdmUserManager: Found object path of user '%s': %s",
                               ...
                               MdmUserManagerFetchUserRequest *request)
{
        ...
        if (call == NULL) {
                g_warning ("MdmUserManager: failed to make FindUserByName('%s') call",
                           request->username);
                goto failed;
        }
...
static void
set_is_loaded (MdmUserManager *manager,
               gboolean        is_loaded)
{
                               ...
                               gpointer        data)
{
        ...
        if (manager->priv->include_usernames != NULL) {
                ...
                for (l = manager->priv->include_usernames; l != NULL; l = l->next) {
                        ...
                        g_debug ("MdmUserManager: Adding included user %s", (char *)l->data);
                        /*
                         * The call to mdm_user_manager_get_user will add the user if it is
                         * valid and not already in the hash.
                         */
                        user = mdm_user_manager_get_user (manager, l->data);
                        if (user == NULL) {
                                g_debug ("MdmUserManager: unable to lookup user '%s'", (char *)l->data);
                        }
                }
        }
}
                             ...
                             MdmUserManagerNewSession *new_session)
{
        ...
        g_debug ("MdmUserManager: Found x11 display of session '%s': %s",
...
static void
get_x11_display_for_new_session (MdmUserManagerNewSession *new_session)
{
        DBusGProxyCall *call;
        ...
        call = dbus_g_proxy_begin_call (new_session->proxy,
                                        ...
                                        G_TYPE_INVALID);
        if (call == NULL) {
                g_warning ("MdmUserManager: failed to make GetX11Display call");
                goto failed;
        }
...
static void
maybe_add_new_session (MdmUserManagerNewSession *new_session)
{
        MdmUserManager *manager;
        struct passwd  *pwent;
        MdmUser        *user;

        manager = MDM_USER_MANAGER (new_session->manager);
        ...
        user = mdm_user_manager_get_user (manager, pwent->pw_name);
        if (user == NULL) {
                return;
        }
        ...
        if (mdm_user_get_login_frequency (user) == 0) {
                _mdm_user_update_login_frequency (user, 1);
        }

        manager->priv->seat.state = MDM_USER_MANAGER_SEAT_STATE_LOADED;
        unload_new_session (new_session);
        return;
...
static void
load_new_session (MdmUserManager *manager,
                  const char     *session_id)
{
        MdmUserManagerNewSession *new_session;

        new_session = g_slice_new0 (MdmUserManagerNewSession);

        new_session->manager = manager;
        new_session->id = g_strdup (session_id);
        new_session->state = MDM_USER_MANAGER_NEW_SESSION_STATE_UNLOADED + 1;
...
seat_session_added (DBusGProxy     *seat_proxy,
                    ...
                    MdmUserManager *manager)
{
        g_debug ("MdmUserManager: Session added: %s", session_id);
                           ...
                           gconstpointer b)
{
        ...
        new_session = (MdmUserManagerNewSession *) a;
                      ...
                      MdmUserManager *manager)
{
        ...
        g_debug ("MdmUserManager: Session removed: %s", session_id);
        ...
        if (found != NULL) {
                ...
                new_session = (MdmUserManagerNewSession *) found->data;

                if (new_session->state > MDM_USER_MANAGER_NEW_SESSION_STATE_GET_X11_DISPLAY) {
                        g_debug ("MdmUserManager: New session for uid %d on "
                                 "x11 display %s removed before fully loading",
                                 (int) new_session->uid, new_session->x11_display);
                } else if (new_session->state > MDM_USER_MANAGER_NEW_SESSION_STATE_GET_UID) {
                        g_debug ("MdmUserManager: New session for uid %d "
                                 "removed before fully loading",
                                 (int) new_session->uid);
                } else {
                        g_debug ("MdmUserManager: New session removed "
...
static void
unload_seat (MdmUserManager *manager)
{
...
static void
load_new_session_incrementally (MdmUserManagerNewSession *new_session)
{
        switch (new_session->state) {
        case MDM_USER_MANAGER_NEW_SESSION_STATE_GET_PROXY:
                get_proxy_for_new_session (new_session);
                break;
        case MDM_USER_MANAGER_NEW_SESSION_STATE_GET_UID:
                get_uid_for_new_session (new_session);
                break;
        case MDM_USER_MANAGER_NEW_SESSION_STATE_GET_X11_DISPLAY:
                get_x11_display_for_new_session (new_session);
                break;
        case MDM_USER_MANAGER_NEW_SESSION_STATE_MAYBE_ADD:
                maybe_add_new_session (new_session);
                break;
        case MDM_USER_MANAGER_NEW_SESSION_STATE_LOADED:
                break;
        default:
...
static void
free_fetch_user_request (MdmUserManagerFetchUserRequest *request)
{
        MdmUserManager *manager;
        ...
        g_slice_free (MdmUserManagerFetchUserRequest, request);
}

static void
give_up_and_fetch_user_locally (MdmUserManager                 *manager,
                                MdmUserManagerFetchUserRequest *request)
{
        ...
        request->state = MDM_USER_MANAGER_GET_USER_STATE_UNFETCHED;
}

static void
on_user_manager_maybe_ready_for_request (MdmUserManager                 *manager,
                                         GParamSpec                     *pspec,
                                         MdmUserManagerFetchUserRequest *request)
{
        if (!manager->priv->is_loaded) {
...
static void
fetch_user_incrementally (MdmUserManagerFetchUserRequest *request)
{
        MdmUserManager *manager;

        g_debug ("MdmUserManager: finding user %s state %d",
                 request->username, request->state);
        manager = request->manager;
        switch (request->state) {
        case MDM_USER_MANAGER_GET_USER_STATE_WAIT_FOR_LOADED:
                if (manager->priv->is_loaded) {
                        request->state++;
                        fetch_user_incrementally (request);
                } else {
                        g_debug ("MdmUserManager: waiting for user manager to load before finding user %s",
                                 request->username);
        ...
        case MDM_USER_MANAGER_GET_USER_STATE_FETCHED:
                g_debug ("MdmUserManager: user %s fetched", request->username);
                _mdm_user_update_from_object_path (request->user, request->object_path);
                break;
        case MDM_USER_MANAGER_GET_USER_STATE_UNFETCHED:
                g_debug ("MdmUserManager: user %s was not fetched", request->username);
                break;
        default:
                g_assert_not_reached ();
        }

        if (request->state == MDM_USER_MANAGER_GET_USER_STATE_FETCHED  ||
            request->state == MDM_USER_MANAGER_GET_USER_STATE_UNFETCHED) {
                g_debug ("MdmUserManager: finished handling request for user %s",
                         request->username);
                                  ...
                                  const char     *username)
{
        ...
        request = g_slice_new0 (MdmUserManagerFetchUserRequest);

        request->manager = manager;
        request->username = g_strdup (username);
        request->user = user;
        request->state = MDM_USER_MANAGER_GET_USER_STATE_UNFETCHED + 1;
...
static void
fetch_user_locally (MdmUserManager *manager,
                    MdmUser        *user,
                    const char     *username)
{
        struct passwd *pwent;
        ...
        if (pwent != NULL) {
                _mdm_user_update_from_pwent (user, pwent);
        }
}

/**
 * mdm_user_manager_get_user:
 * @manager: the manager to query.
 * @username: the login name of the user to get.
 *
 * Retrieves a pointer to the #MdmUser object for the login @username
 * from @manager. Trying to use this object before its
 * #MdmUser:is-loaded property is %TRUE will result in undefined
 * behavior.
 *
 * Returns: (transfer none): #MdmUser object
 **/
MdmUser *
...
/**
 * mdm_user_manager_get_user_by_uid:
 * @manager: the manager to query.
 * @uid: the uid of the user to get.
 *
 * Retrieves a pointer to the #MdmUser object for the uid @uid
 * from @manager. Trying to use this object before its
 * #MdmUser:is-loaded property is %TRUE will result in undefined
 * behavior.
 *
                                  ...
                                  gulong          uid)
{
        ...
        return mdm_user_manager_get_user (manager, pwent->pw_name);
}
...
/**
 * mdm_user_manager_list_users:
 * @manager: a #MdmUserManager
 *
 * Get a list of system user accounts
 *
 * Returns: (element-type MdmUser) (transfer full): List of #MdmUser objects
 */
GSList *
                         ...
                         const char     *line)
{
        ...
        _mdm_user_update_login_frequency (user, frequency);
        g_free (username);
}

static void
maybe_set_is_loaded (MdmUserManager *manager)
{
        if (manager->priv->is_loaded) {
                return;
        }
         ...
         * or failed to load.
         */
        if (manager->priv->seat.state != MDM_USER_MANAGER_SEAT_STATE_LOADED
            && manager->priv->seat.state != MDM_USER_MANAGER_SEAT_STATE_UNLOADED) {
                return;
        }
...
static gboolean
ck_history_watchdog (MdmUserManager *manager)
{
        if (manager->priv->ck_history_pid > 0) {
...
static gboolean
load_ck_history (MdmUserManager *manager)
{
        char       *command;
        char       *since;
        const char *seat_id;
                    ...
                    GSList    **removed_users)
{
        ...
        while (g_hash_table_iter_next (&iter, (gpointer *) &name, (gpointer *) &user)) {
                ...
                _mdm_user_update_from_pwent (user, pwent);
                g_hash_table_insert (new_users_by_name, (char *)mdm_user_get_user_name (user), g_object_ref (user));
        }
        ...
        if (include_users != NULL) {
                ...
                for (l = include_users; l != NULL; l = l->next) {
                        ...
                        g_object_freeze_notify (G_OBJECT (user));
                        _mdm_user_update_from_pwent (user, pwent);
                        g_hash_table_insert (new_users_by_name, (char *)mdm_user_get_user_name (user), user);
                }
        }
        ...
        if (include_all != TRUE) {
                ...
                for (pwent = fgetpwent (fp);
                     ...
                     pwent = fgetpwent (fp)) {
                        ...
                        /* always exclude the "mdm" user. */
                        if (strcmp (pwent->pw_name, MDM_USERNAME) == 0) {
                                continue;
                        }
...
reload_passwd_job_done (PasswdData *data)
{
        ...
        g_debug ("MdmUserManager: done reloading passwd file");
...
do_reload_passwd_job (GIOSchedulerJob *job,
                      ...
                      PasswdData      *data)
{
        g_debug ("MdmUserManager: reloading passwd file worker");
...
static void
schedule_reload_passwd (MdmUserManager *manager)
{
        ...
        g_debug ("MdmUserManager: scheduling a passwd file update");
...
static void
load_sessions_from_array (MdmUserManager     *manager,
                          ...
                          MdmUserManager *manager)
{
        ...
        res = dbus_g_proxy_end_call (proxy,
                                     call,
                                     &error,
                                     MDM_DBUS_TYPE_G_OBJECT_PATH_ARRAY,
                                     &sessions,
                                     G_TYPE_INVALID);
...
static void
load_sessions (MdmUserManager *manager)
{
        DBusGProxyCall *call;

        if (manager->priv->seat.proxy == NULL) {
                g_debug ("MdmUserManager: no seat proxy; can't load sessions");
                return;
        }
        ...
        if (call == NULL) {
                g_warning ("MdmUserManager: failed to make GetSessions call");
                return;
        }
...
static void
load_users_manually (MdmUserManager *manager)
{
        gboolean res;
...
static void
load_seat_incrementally (MdmUserManager *manager)
{
        g_assert (manager->priv->seat.proxy == NULL);

        switch (manager->priv->seat.state) {
        case MDM_USER_MANAGER_SEAT_STATE_GET_SESSION_ID:
                get_current_session_id (manager);
                break;
        case MDM_USER_MANAGER_SEAT_STATE_GET_ID:
                get_seat_id_for_current_session (manager);
                break;
        case MDM_USER_MANAGER_SEAT_STATE_GET_PROXY:
                get_seat_proxy (manager);
                break;
        case MDM_USER_MANAGER_SEAT_STATE_LOADED:
                break;
        default:
                g_assert_not_reached ();
        }
...
static gboolean
load_idle (MdmUserManager *manager)
{
        manager->priv->seat.state = MDM_USER_MANAGER_SEAT_STATE_UNLOADED + 1;
        load_seat_incrementally (manager);
        load_users (manager);
...
static void
queue_load_seat_and_users (MdmUserManager *manager)
{
...
static gboolean
reload_passwd_idle (MdmUserManager *manager)
{
        schedule_reload_passwd (manager);
...
static void
queue_reload_passwd (MdmUserManager *manager)
{
...
static void
mdm_user_manager_get_property (GObject        *object,
                               guint           prop_id,
                               GValue         *value,
                               GParamSpec     *pspec)
{
...
static void
set_include_usernames (MdmUserManager *manager,
                       GSList         *list)
{
        if (manager->priv->include_usernames != NULL) {
...
static void
set_exclude_usernames (MdmUserManager *manager,
                       GSList         *list)
{
        if (manager->priv->exclude_usernames != NULL) {
...
static void
set_include_all (MdmUserManager *manager,
                 gboolean        all)
{
...
static void
mdm_user_manager_set_property (GObject        *object,
                               guint           prop_id,
                               const GValue   *value,
                               GParamSpec     *pspec)
{
...
static void
monitor_local_users (MdmUserManager *manager)
{
        GFile *file;
        GError *error;

        g_debug ("MdmUserManager: Monitoring local users");
...
mdm_user_manager_class_init (MdmUserManagerClass *klass)
{
        ...
        object_class->get_property = mdm_user_manager_get_property;
        object_class->set_property = mdm_user_manager_set_property;
        ...
        signals [USER_CHANGED] =
                g_signal_new ("user-changed",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MdmUserManagerClass, user_changed),
                              NULL, NULL,
                              g_cclosure_marshal_VOID__OBJECT,
                              G_TYPE_NONE, 1, MDM_TYPE_USER);
...
/**
 * mdm_user_manager_queue_load:
 * @manager: a #MdmUserManager
 *
 * Queue loading users into user manager. This must be called, and the
 * #MdmUserManager:is-loaded property must be %TRUE before calling
 * mdm_user_manager_list_users()
 */
void
mdm_user_manager_queue_load (MdmUserManager *manager)
{
        g_return_if_fail (MDM_IS_USER_MANAGER (manager));
