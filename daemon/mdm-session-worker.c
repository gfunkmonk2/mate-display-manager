...
static void
mdm_session_worker_set_is_failsafe (MdmSessionWorker *worker,
                                    gboolean         is_failsafe)
{
        mdm_session_settings_set_is_failsafe (worker->priv->user_settings,
                                              is_failsafe);
}
...
static void
on_set_is_session_failsafe (MdmSessionWorker *worker,
                            DBusMessage      *message)
{
        DBusError   error;
        ...
        res = dbus_message_get_args (message,
                                     ...
                                     DBUS_TYPE_INVALID);
        if (res) {
                g_debug ("MdmSessionWorker: is_failsafe for this session set to %i", is_failsafe);
                mdm_session_worker_set_is_failsafe (worker, is_failsafe);
        } else {
                g_warning ("Unable to get arguments: %s", error.message);
                dbus_error_free (&error);
        }
                            ...
                            dbus_bool_t     local_interface)
{
        ...
        } else if (dbus_message_is_signal (message, MDM_SESSION_DBUS_INTERFACE, "SetIsFailsafe")) {
