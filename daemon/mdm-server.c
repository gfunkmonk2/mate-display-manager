                  ...
                  const char *vtarg)
{
	 ...
	 * usually get gettys after mdm startup, which interfere with X) */
        int f = open ("/var/run/mdm/firstserver.stamp", O_CREAT|O_WRONLY|O_EXCL, 0666);
        if (f >= 0)
                close (f);
...
mdm_server_start (MdmServer *server)
{
         ...
         * gettys after mdm startup, which interfere with X */
	firstserver = access ("/var/run/mdm/firstserver.stamp", F_OK) < 0;

        /* fork X server process */
        res = mdm_server_spawn (server, firstserver ? "vt7" : NULL);
