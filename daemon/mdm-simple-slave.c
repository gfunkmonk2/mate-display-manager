...
static void
mdm_simple_slave_set_username (MdmSimpleSlave *slave,
                               const char     *username)
{
        g_return_if_fail (MDM_IS_SIMPLE_SLAVE (slave));
                                 ...
                                 MdmSimpleSlave *slave)
{
        ...
        if (username) {
                g_debug ("MdmSimpleSlave: **user '%s'", username);
                if (slave->priv->greeter_server != NULL) {
                        g_debug ("MdmSimpleSlave: begin login for user '%s'", username);
                        mdm_simple_slave_set_username (slave, NULL);
                        mdm_greeter_server_select_user (slave->priv->greeter_server, username);
                }
                g_free (username);
                return;
        }
        g_debug ("MdmSimpleSlave: **nouser");
                               ...
                               GParamSpec   *pspec)
{
        ...
        self = MDM_SIMPLE_SLAVE (object);

        switch (prop_id) {
        case PROP_USERNAME:
	            mdm_simple_slave_set_username (self, g_value_get_string (value));
                break;
        default:
...
MdmSlave *
mdm_simple_slave_new (const char *id, const char *username)
{
        GObject *object;
